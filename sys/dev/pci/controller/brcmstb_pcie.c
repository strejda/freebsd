...
brcm_pcie_setup_inbound(struct brcm_pcie_softc *sc)
{
	...
	for (i = 0; i < sc->ofw_pci.sc_ndmarange; i++) {
		if (min_pcie > sc->ofw_pci.sc_dmarange[i].pci)
			min_pcie = sc->ofw_pci.sc_dmarange[i].pci;
		tot_size += sc->ofw_pci.sc_dmarange[i].size;

		if (sc->soc->type == BCM2712) {
			*rangep = sc->ofw_pci.sc_dmarange[i];
    ...
    bus_size_t size)
{
	...
	for (i = 0; i < sc->ofw_pci.sc_ndmarange; i++) {
		pci_start  = sc->ofw_pci.sc_dmarange[i].pci;
		host_start = sc->ofw_pci.sc_dmarange[i].host;
		host_end = host_start + sc->ofw_pci.sc_dmarange[i].size;
...
static device_method_t brcm_pcie_methods[] = {
	...
	DEVMETHOD(pic_disable_intr,		brcm_pcie_msi_disable_intr),
	DEVMETHOD(pic_enable_intr,		brcm_pcie_msi_enable_intr),
	DEVMETHOD(pic_setup_intr,		brcm_pcie_msi_setup_intr),
	DEVMETHOD(pic_teardown_intr,		brcm_pcie_msi_teardown_intr),
	DEVMETHOD(pic_post_filter,		brcm_pcie_msi_post_filter),
	DEVMETHOD(pic_post_ithread,		brcm_pcie_msi_post_ithread),
	DEVMETHOD(pic_pre_ithread,		brcm_pcie_msi_pre_ithread),
#endif
