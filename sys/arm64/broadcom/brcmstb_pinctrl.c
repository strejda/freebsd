...
struct brcm_pinctrl_soc {
	int			type;
};


struct brcm_pinctrl_softc {
	device_t		dev;
	phandle_t		node;
	struct brcm_pinctrl_soc	*soc;

	struct resource		*mem_res;
	struct mtx		mtx;
};

static struct brcm_pinctrl_soc soc_bcm2712c0 = {
	.type = BCM2712C0,
};

static struct brcm_pinctrl_soc soc_bcm2712c0_aon = {
	.type = BCM2712C0_AON,
};

static struct brcm_pinctrl_soc soc_bcm2712d0 = {
	.type = BCM2712D0,
};

static struct brcm_pinctrl_soc soc_bcm2712d0_aon = {
	.type = BCM2712D0_AON,
};
...
static struct brcm_pinctrl_gpio rk3288_gpio_bank[] = {
	RK_GPIO(0, "gpio0"),
	RK_GPIO(1, "gpio1"),
	RK_GPIO(2, "gpio2"),
	RK_GPIO(3, "gpio3"),
...
static struct brcm_pinctrl_bank rk3288_iomux_bank[] = {
	/*    bank sub  offs   nbits */
	/* PMU */
...
static struct brcm_pinctrl_pin_fixup rk3288_pin_fixup[] = {
};

static struct brcm_pinctrl_pin_drive rk3288_pin_drive[] = {
...
static uint32_t
rk3288_get_pd_offset(struct brcm_pinctrl_softc *sc, uint32_t bank)
{
	if (bank == 0)
		return (0x064);		/* PMU */
	return (0x130);
...
rk3288_get_syscon(struct brcm_pinctrl_softc *sc, uint32_t bank)
{
	if (bank == 0)
		return (sc->pmu);
	return (sc->grf);
...
struct brcm_pinctrl_conf rk3288_conf = {
	.chip = RK_CHIP_RK3288,
...
static struct brcm_pinctrl_gpio rk3328_gpio_bank[] = {
	RK_GPIO(0, "gpio0"),
	RK_GPIO(1, "gpio1"),
	RK_GPIO(2, "gpio2"),
	RK_GPIO(3, "gpio3"),
...
static struct brcm_pinctrl_bank rk3328_iomux_bank[] = {
	/*    bank sub offs nbits */
...
static struct brcm_pinctrl_pin_fixup rk3328_pin_fixup[] = {
...
static struct brcm_pinctrl_pin_drive rk3328_pin_drive[] = {
	/*       bank sub  offs val ma */
...
static uint32_t
rk3328_get_pd_offset(struct brcm_pinctrl_softc *sc, uint32_t bank)
{
	return (0x100);
}

static struct syscon *
rk3328_get_syscon(struct brcm_pinctrl_softc *sc, uint32_t bank)
{
	return (sc->grf);
}

struct brcm_pinctrl_conf rk3328_conf = {
	.chip = RK_CHIP_RK3328,
...
static struct brcm_pinctrl_gpio rk3399_gpio_bank[] = {
	RK_GPIO(0, "gpio0"),
	RK_GPIO(1, "gpio1"),
	RK_GPIO(2, "gpio2"),
	RK_GPIO(3, "gpio3"),
...
static struct brcm_pinctrl_bank rk3399_iomux_bank[] = {
	/*    bank sub  offs   nbits */
	RK_IOMUX(0, 0, 0x0000, 2),
	RK_IOMUX(0, 1, 0x0004, 2),
	RK_IOMUX(0, 2, 0x0008, 2),
...
static struct brcm_pinctrl_pin_fixup rk3399_pin_fixup[] = {};

static struct brcm_pinctrl_pin_drive rk3399_pin_drive[] = {
	/*       bank sub offs val ma */
	/* GPIO0A */
...
static uint32_t
rk3399_get_pd_offset(struct brcm_pinctrl_softc *sc, uint32_t bank)
{
	if (bank < 2)
		return (0x40);
...
static struct syscon *
rk3399_get_syscon(struct brcm_pinctrl_softc *sc, uint32_t bank)
{
	if (bank < 2)
		return (sc->pmu);
...
struct brcm_pinctrl_conf rk3399_conf = {
	.chip = RK_CHIP_RK3399,
...
static struct brcm_pinctrl_gpio rk3568_gpio_bank[] = {
	RK_GPIO(0, "gpio0"),
	RK_GPIO(1, "gpio1"),
	RK_GPIO(2, "gpio2"),
	RK_GPIO(3, "gpio3"),
...
static struct brcm_pinctrl_bank rk3568_iomux_bank[] = {
	/*    bank sub  offs   nbits */
...
static struct brcm_pinctrl_pin_fixup rk3568_pin_fixup[] = {};

static struct brcm_pinctrl_pin_drive rk3568_pin_drive[] = {
	/*       bank sub offs val ma */
	/* GPIO0A */
...
static uint32_t
rk3568_get_pd_offset(struct brcm_pinctrl_softc *sc, uint32_t bank)
{
...
static struct syscon *
rk3568_get_syscon(struct brcm_pinctrl_softc *sc, uint32_t bank)
{
...
struct brcm_pinctrl_conf rk3568_conf = {
	.chip = RK_CHIP_RK3568,
...
static struct brcm_pinctrl_bank rk3588_iomux_bank[] = {
	/*    bank sub  offs   nbits */
...
static struct brcm_pinctrl_pin_fixup rk3588_pin_fixup[] = {
	/*      bank  pin reg  bit  mask */
...
static struct brcm_pinctrl_gpio rk3588_gpio_bank[] = {
	RK_GPIO(0, "gpio0"),
	RK_GPIO(1, "gpio1"),
	RK_GPIO(2, "gpio2"),
	RK_GPIO(3, "gpio3"),
...
static struct syscon *
rk3588_get_syscon(struct brcm_pinctrl_softc *sc, uint32_t bank)
{
...
static int
rk3588_get_pd_reg(struct brcm_pinctrl_softc *sc, uint32_t bank, uint32_t pin,
...
static int
rk3588_get_bias_reg(struct brcm_pinctrl_softc *sc, uint32_t bank, uint32_t pin,
    uint32_t *reg, uint32_t *bit, uint32_t *mask)
{
	if (bank == 0) {
		if (pin >= RK_PA0 && pin <= RK_PB3) {
...
struct brcm_pinctrl_conf rk3588_conf = {
	.chip = RK_CHIP_RK3588,
...
brcm_pinctrl_parse_drive(struct brcm_pinctrl_softc *sc, phandle_t node,
...
static void
brcm_pinctrl_get_fixup(struct brcm_pinctrl_softc *sc, uint32_t bank, uint32_t pin,
...
static int
brcm_pinctrl_handle_io(struct brcm_pinctrl_softc *sc, phandle_t node, uint32_t bank,
uint32_t pin)
{
	bool have_cfg, have_direction, have_value;
	uint32_t  direction_value, pin_value;
	struct brcm_pinctrl_gpio *gpio;
	int i, rv;
...
static void
brcm_pinctrl_configure_pin(struct brcm_pinctrl_softc *sc, uint32_t *pindata)
{
	phandle_t pin_conf;
...
//device_printf(sc->dev, "%s: bank: %u, pin: %u fnc: %u\n", __func__, bank, pin, function);
	...
	rv = brcm_pinctrl_handle_io(sc, pin_conf, bank, pin);
	...
	if (sc->conf->chip == RK_CHIP_RK3568) {
		...
		rv = brcm_pinctrl_parse_drive(sc, pin_conf, bank, pin, &reg, &bit,
		    &mask, &drive);
...
//rv, reg, bit,mask, drive);
	...
	brcm_pinctrl_get_fixup(sc, bank, pin, &reg, &mask, &bit);
...
static int
brcm_pinctrl_configure_pins(device_t dev, phandle_t cfgxref)
{
	struct brcm_pinctrl_softc *sc;
	phandle_t node;
	uint32_t *pins;
	int i, npins;
...
//		brcm_pinctrl_configure_pin(sc, pins + i);
	}
...
#if 0
static int
brcm_pinctrl_is_gpio_locked(struct brcm_pinctrl_softc *sc, struct syscon *syscon,
...
static int
brcm_pinctrl_get_bank(struct brcm_pinctrl_softc *sc, device_t gpio, int *bank)
{
	int i;
...
static int
brcm_pinctrl_is_gpio(device_t pinctrl, device_t gpio, uint32_t pin, bool *is_gpio)
{
	struct brcm_pinctrl_softc *sc;
	struct syscon *syscon;
	int bank;
	...
	rv = brcm_pinctrl_get_bank(sc, gpio, &bank);
	if (rv != 0)
		goto done;

	syscon = sc->conf->get_syscon(sc, bank);
	rv = brcm_pinctrl_is_gpio_locked(sc, syscon, bank, pin, is_gpio);
...
brcm_pinctrl_attach(device_t dev)
{
	...
	sc->soc = (struct brcm_pinctrl_soc *)ofw_bus_search_compatible(dev,
...
static device_method_t brcm_pinctrl_methods[] = {
	...
	DEVMETHOD(fdt_pinctrl_configure,	brcm_pinctrl_configure_pins),

	DEVMETHOD_END
};

DEFINE_CLASS_0(brcmstb_pinctrl, brcmstb_pinctrl_driver, brcm_pinctrl_methods,
    sizeof(struct brcm_pinctrl_softc));
